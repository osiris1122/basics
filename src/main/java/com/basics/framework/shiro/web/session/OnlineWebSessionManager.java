package com.basics.framework.shiro.web.session;
...
import com.basics.system.service.impl.UserOnlineServiceImpl;
...
public class OnlineWebSessionManager extends DefaultWebSessionManager
{
    ...
    public void validateSessions()
    {
        ...
        List<UserOnline> userOnlineList = userOnlineService.list(new QueryWrapper<UserOnline>().le("LAST_ACCESS_TIME", expiredDate));
        // 批量过期删除
        ...
        if (needOfflineIdList.size() > 0)
        {
            try
            {
                for (String id : needOfflineIdList) {
                    userOnlineService.removeById(id);
                }
            }
            catch (Exception e)
            {
